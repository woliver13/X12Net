...
public class X12TokenizerTests
{
    ...
    public void Tokenizer_splits_segment_into_element_tokens()
    {
        ...
        tokens[0].Type.ShouldBe(X12TokenType.SegmentId);
        tokens[0].Value.ShouldBe("NM1");

        tokens[1].Type.ShouldBe(X12TokenType.ElementData);
        tokens[1].Value.ShouldBe("IL");

        tokens[2].Type.ShouldBe(X12TokenType.ElementData);
        tokens[2].Value.ShouldBe("1");

        tokens[3].Type.ShouldBe(X12TokenType.ElementData);
        tokens[3].Value.ShouldBe("DOE");

        tokens[4].Type.ShouldBe(X12TokenType.ElementData);
        tokens[4].Value.ShouldBe("JOHN");
    }
    ...
    public void Tokenizer_detects_delimiters_from_ISA_header()
    {
        ...
        d.ElementSeparator.ShouldBe('|');    // position 3
        d.ComponentSeparator.ShouldBe('^');  // ISA16
        d.SegmentTerminator.ShouldBe('\n');  // char after ISA16
    }
    ...
    public void Tokenizer_uses_detected_delimiters_for_subsequent_segments()
    {
        ...
        tokens[gsIndex + 1].Value.ShouldBe("FA");
        tokens[gsIndex + 1].Type.ShouldBe(X12TokenType.ElementData);
    }
    ...
    public void Tokenizer_emits_ComponentData_token_for_component_separator()
    {
        ...
        tokens[0].Type.ShouldBe(X12TokenType.SegmentId);    // CLM
        tokens[1].Type.ShouldBe(X12TokenType.ElementData);  // 1234
        tokens[2].Type.ShouldBe(X12TokenType.ElementData);  // B   ← first in composite = ElementData
        tokens[3].Type.ShouldBe(X12TokenType.ComponentData); // 1  ← component
        tokens[4].Type.ShouldBe(X12TokenType.ComponentData); // 1  ← component
        tokens[2].Value.ShouldBe("B");
        tokens[3].Value.ShouldBe("1");
        tokens[4].Value.ShouldBe("1");
    }
    ...
    public void Tokenizer_emits_SegmentTerminator_token_at_end_of_segment()
    {
        ...
        terminator.Type.ShouldBe(X12TokenType.SegmentTerminator);
        terminator.Value.ShouldBe("~");
    }
}
